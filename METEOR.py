...
def get_meteor(ref_path, gen_path, is_sentence=False):
    ...
    meteor_lst = [meteor_score([ref_sentence], gen_sentence) for ref_sentence, gen_sentence in zip(ref_sentence_lst, gen_sentence_lst)]
    meteor_score = np.mean(meteor_lst)
    return meteor_score*100

if __name__ == "__main__":

    ##### get parameters #####
    parser = argparse.ArgumentParser(description='calculate Meteor')
